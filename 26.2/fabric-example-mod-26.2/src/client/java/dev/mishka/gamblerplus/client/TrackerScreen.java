...
public final class TrackerScreen extends Screen {
	...
	public TrackerScreen(Config config, Stats stats) {
		super(Minecraft.getInstance(), Minecraft.getInstance().font, Component.literal("Gambler Plus"));
		this.config = config;
		this.stats  = stats;
	...
	public void extractRenderState(GuiGraphicsExtractor ctx, int mx, int my, float delta) {
		...
		ctx.text(font, credits, px + (PANEL_W - cw) / 2, py + PANEL_H + 6, Theme.TEXT_DIM, false);
		pose.popMatrix();
	...
	private void drawHeader(GuiGraphicsExtractor ctx) {
		...
		ctx.text(font, "Gambler",     textX, textY, Theme.TEXT, false);
		int nameW = font.width("Gambler ");
		ctx.text(font, "Plus", textX + nameW, textY, Theme.BRAND, false);

		String modeTag = config.gamblingMode() ? "TRACKING" : "PAUSED";
		int mw = font.width(modeTag);
		int rx = hx + PANEL_W - mw - 12;
		ctx.text(font, modeTag, rx, textY, config.gamblingMode() ? Theme.GAIN : Theme.TEXT_DIM, false);
	}

	private void drawArrowGameToggle(GuiGraphicsExtractor ctx, int mx, int my, int belowPanelY) {
		int y = belowPanelY + 4;
		int x = px;
		toggle(ctx, "arrow game support", config.arrowGameSupport(), x, y, mx, my, HitId.ARROW_GAME);
	}

	private void drawDiscord(GuiGraphicsExtractor ctx, int mx, int my, int belowPanelY) {
		int bw = 58, bh = 14;
		...
		ctx.text(font, label, bx + (bw - lw) / 2, by + (bh - font.lineHeight) / 2 + 1, hover ? Theme.TEXT : 0xFFDDE1FF, false);
		hitboxes[HitId.OPEN_DISCORD.ordinal()] = new int[]{bx, by, bx + bw, by + bh};
	}

	private void drawSummary(GuiGraphicsExtractor ctx, int mx, int my) {
		int sx = px + 14, sy = py + 40;
		int colW = 152;

		ctx.text(font, "SESSION",  sx,          sy, Theme.TEXT_DIM, false);
		ctx.text(font, "ALL TIME", sx + colW,   sy, Theme.TEXT_DIM, false);
		ctx.text(font, "STREAKS",  sx + colW*2, sy, Theme.TEXT_DIM, false);

		long snet = stats.sessionNet();
		long anet = stats.allTimeNet();

		ctx.text(font, AmountFormat.signed(snet), sx,          sy + 12, Theme.color(snet), false);
		ctx.text(font, AmountFormat.signed(anet), sx + colW,   sy + 12, Theme.color(anet), false);
		ctx.text(font, stats.lossStreak() + "L / " + stats.winStreak() + "W",
				sx + colW*2, sy + 12,
	...
	private void subLine(GuiGraphicsExtractor ctx, int x, int y, String label, String value, int valueColor) {
		ctx.text(font, label, x, y, Theme.TEXT_DIM, false);
		int lw = font.width(label);
		ctx.text(font, value, x + lw + 6, y, valueColor, false);
	}

	private void drawTriggerControl(GuiGraphicsExtractor ctx, int x, int y, int mx, int my) {
		ctx.text(font, "trigger", x, y, Theme.TEXT_DIM, false);
		...
		ctx.text(font, val, cursor, y, Theme.TEXT, false);
		cursor += vw + 4;
		miniButton(ctx, "+", cursor, btnY, btnW, btnH, mx, my, HitId.STREAK_UP, canUp);
	}

	private void drawViewTab(GuiGraphicsExtractor ctx, String label, int x, int y, int w, int h, boolean active, int mx, int my, HitId id) {
		...
		int lw = font.width(label);
		ctx.text(font, label, x + (w - lw) / 2, y + (h - font.lineHeight) / 2 + 1, active ? Theme.BRAND : Theme.TEXT_MUTED, false);
		hitboxes[id.ordinal()] = new int[]{x, y, x + w, y + h};
	}

	private void miniButton(GuiGraphicsExtractor ctx, String label, int x, int y, int w, int h, int mx, int my, HitId id, boolean enabled) {
		...
		ctx.text(font, label, x + (w - lw) / 2, y + (h - font.lineHeight) / 2 + 1, color, false);
		hitboxes[id.ordinal()] = enabled ? new int[]{x, y, x + w, y + h} : null;
	}

	private void drawLog(GuiGraphicsExtractor ctx, int mx, int my) {
		int lx = px + 14;
		int lw = PANEL_W - 28;
		...
		ctx.text(font, header, lx, headerY, Theme.TEXT_DIM, false);
		...
		if (log.isEmpty()) {
			...
			ctx.text(font, empty, lx + (lw - ew) / 2, listTop + lh/2 - 4, Theme.TEXT_DIM, false);
			return;
		}
		...
		for (int i = 0; i < max; i++) {
			...
			ctx.text(font, time, lx + 8,   rowY, Theme.TEXT_DIM, false);
			ctx.text(font, dir,  lx + 60,  rowY, dirColor, false);
			ctx.text(font, e.player(), lx + 90, rowY, Theme.TEXT, false);
			int aw = font.width(amt);
			int amtX = lx + lw - 20 - aw;
			ctx.text(font, amt, amtX, rowY, dirColor, false);
			String mult = Multiplier.labelFor(log, start + i);
			if (mult != null) {
				int mw = font.width(mult);
				ctx.text(font, mult, amtX - mw - 4, rowY, Theme.WARN, false);
			}
			if (!e.incoming()) {
				int px2 = amtX - 34;
				int px3 = amtX - 18;
				...
				ctx.text(font, "2x", px2, rowY, h2 ? Theme.GAIN : Theme.BRAND, false);
				ctx.text(font, "3x", px3, rowY, h3 ? Theme.GAIN : Theme.BRAND, false);
			...
			ctx.text(font, "x", xBtnX, rowY, xHover ? Theme.LOSS : Theme.TEXT_DIM, false);
	...
	private void drawFooter(GuiGraphicsExtractor ctx, int mx, int my) {
		...
		ctx.text(font, hint, px + PANEL_W - 14 - kw, row2Y + 2, Theme.TEXT_DIM, false);
	...
	private void drawSessionsPanel(GuiGraphicsExtractor ctx, int mx, int my) {
		int sx = sessionsX;
		int sy = py;
		int sw = SIDE_W;
		int sh = PANEL_H;
		...
		ctx.text(font, "SESSIONS", sx + 12, sy + 9, Theme.TEXT, false);
		...
		if (cur != null) {
			...
			ctx.text(font, cur.label(), cardX + 8, contentY + 5, Theme.TEXT, false);
			String durationStr = "for " + formatDuration(System.currentTimeMillis() - cur.startedAt());
			ctx.text(font, durationStr, cardX + 8, contentY + 17, Theme.TEXT_DIM, false);
			String netStr = AmountFormat.signed(cur.net());
			ctx.text(font, netStr, cardX + 8, contentY + 30, Theme.color(cur.net()), false);
			String recStr = cur.inCount() + "W / " + cur.outCount() + "L";
			int rw = font.width(recStr);
			ctx.text(font, recStr, cardX + cardW - 8 - rw, contentY + 30, Theme.TEXT_MUTED, false);
			...
			ctx.text(font, msg, cardX + (cardW - mw) / 2, contentY + 8, Theme.TEXT_DIM, false);
		...
		ctx.text(font, "PAST", sx + 12, contentY, Theme.TEXT_DIM, false);
		contentY += 12;
		...
		if (past.isEmpty()) {
			...
			int w = font.width(s);
			ctx.text(font, s, sx + (sw - w) / 2, listTop + 6, Theme.TEXT_DIM, false);
		} else {
			...
			for (int i = 0; i < max; i++) {
				...
				ctx.text(font, DATE_FMT.format(Instant.ofEpochMilli(s.startedAt())), sx + 10, rowY, Theme.TEXT_DIM, false);
				String net = AmountFormat.signed(s.net());
				int nw = font.width(net);
				ctx.text(font, net, sx + sw - 22 - nw, rowY, Theme.color(s.net()), false);
				String meta = s.inCount() + "W/" + s.outCount() + "L - " + formatDuration(s.durationMs());
				ctx.text(font, meta, sx + 10, rowY + 10, Theme.TEXT_MUTED, false);

				ctx.text(font, "x", delX + 2, delY + 1, deleteHover ? Theme.LOSS : Theme.TEXT_DIM, false);
	...
	private void drawVerifyPanel(GuiGraphicsExtractor ctx, int mx, int my) {
		int vx = verifyX;
		int vy = py;
		int vw = SIDE_W;
		int vh = PANEL_H;
		...
		ctx.text(font, "SETTINGS", vx + 12, vy + 9, Theme.TEXT, false);

		int fieldX = vx + 12;
		int fieldW = vw - 24;
		int contentY = vy + 32;

		ctx.text(font, "verify large payments", fieldX, contentY, Theme.TEXT_DIM, false);
		contentY += 12;
		...
		ctx.text(font, shown, shownX, shownY, Theme.TEXT, false);
		...
		String lo = "1K";
		String hi = "10B";
		ctx.text(font, lo, fieldX, trackY + trackH + 3, Theme.TEXT_DIM, false);
		int hiW = font.width(hi);
		ctx.text(font, hi, fieldX + fieldW - hiW, trackY + trackH + 3, Theme.TEXT_DIM, false);
		contentY += 26;

		ctx.fill(vx + 12, contentY, vx + vw - 12, contentY + 1, Theme.DIVIDER);
		contentY += 8;

		ctx.text(font, "rakeback", fieldX, contentY, Theme.TEXT_DIM, false);
		contentY += 12;
		...
		ctx.text(font, rateStr, fieldX + (fieldW - rw) / 2, contentY + (fieldH - font.lineHeight) / 2, Theme.TEXT, false);
		contentY += fieldH + 4;
	...
	private int toggle(GuiGraphicsExtractor ctx, String label, boolean on, int x, int y, int mx, int my, HitId id) {
		int trackW = 28;
		int trackH = 14;
		int r = trackH / 2;
		int lw = font.width(label);
		...
		ctx.text(font, label, x + trackW + 6, labelY, on ? Theme.TEXT : Theme.TEXT_MUTED, false);
	...
	private void fillPill(GuiGraphicsExtractor ctx, int x, int y, int w, int h, int color) {
		int r = h / 2;
	...
	private void fillCircle(GuiGraphicsExtractor ctx, int cx, int cy, int r, int color) {
	...
	private int pill(GuiGraphicsExtractor ctx, String label, String value, int x, int y, int mx, int my, HitId id) {
		...
		ctx.text(font, label + ":", x + 7, y + 3, Theme.TEXT_DIM, false);
		int labelW = font.width(label + ": ");
		ctx.text(font, value, x + 7 + labelW, y + 3, Theme.TEXT, false);
		hitboxes[id.ordinal()] = new int[]{x, y, x + pw, y + ph};
		return x + pw;
	}

	private void button(GuiGraphicsExtractor ctx, String label, int x, int y, int w, int h, int mx, int my, HitId id, int accent) {
