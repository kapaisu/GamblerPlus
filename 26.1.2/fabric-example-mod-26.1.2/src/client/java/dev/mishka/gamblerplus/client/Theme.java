...
public final class Theme {
	...
	public static void roundRect(GuiGraphicsExtractor ctx, int x1, int y1, int x2, int y2, int radius, int color) {
	...
	public static void roundPanel(GuiGraphicsExtractor ctx, int x1, int y1, int x2, int y2, int radius, int fill, int edge) {
		roundRect(ctx, x1, y1, x2, y2, radius, edge);
		int innerR = Math.max(0, radius - 1);
		roundRect(ctx, x1 + 1, y1 + 1, x2 - 1, y2 - 1, innerR, fill);
	}

	public static void roundOutline(GuiGraphicsExtractor ctx, int x1, int y1, int x2, int y2, int radius, int color) {
		int r = Math.min(radius, Math.min((x2 - x1) / 2, (y2 - y1) / 2));
		if (r <= 0) {
	...
	private static void fillQuarter(GuiGraphicsExtractor ctx, int cx, int cy, int r, int color, boolean left, boolean top) {
	...
	private static void strokeQuarter(GuiGraphicsExtractor ctx, int cx, int cy, int r, int color, boolean left, boolean top) {
		int prev = r + 1;
		for (int dy = 0; dy < r; dy++) {
			double h = dy + 0.5;
			int dx = (int) Math.ceil(Math.sqrt(r * r - h * h));
