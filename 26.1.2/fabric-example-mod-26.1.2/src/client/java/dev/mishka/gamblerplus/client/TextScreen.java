...
public final class TextScreen extends Screen {
	...
	public void extractRenderState(GuiGraphicsExtractor ctx, int mxRaw, int myRaw, float delta) {
		...
		ctx.text(font, "ENTRIES", listX + 6, listY + 4, Theme.TEXT_DIM, false);
		...
		for (int i = listScroll; i < entries.size(); i++) {
			...
			ctx.text(font, label, listX + 8, rowY + (rowH - font.lineHeight) / 2 + 1, isSel ? Theme.TEXT : Theme.TEXT_MUTED, false);
		...
		if (entries.isEmpty()) {
			ctx.text(font, "no entries yet", listX + 8, listY + 20, Theme.TEXT_DIM, false);
		}
		...
		if (selected >= 0 && selected < entries.size()) {
			...
			int contentY = listY;
			ctx.text(font, "text", editX, contentY, Theme.TEXT_DIM, false);
			int fY = contentY + 12;
			int fH = 20;
			...
			int r2Y = fY + fH + 8;
			ctx.text(font, "font", editX, r2Y, Theme.TEXT_DIM, false);
			...
			for (int i = 0; i < 3; i++) {
				int y = sliderY + i * 16;
				ctx.text(font, labels[i], editX, y, Theme.TEXT_DIM, false);
				...
				ctx.text(font, val, trackX + sliderW + 8, y - 1, Theme.TEXT, false);
			...
			ctx.text(font, "PREVIEW", editX + 6, previewY + 3, Theme.TEXT_DIM, false);
			...
			ctx.text(font, preview, editX + 8, previewY + 16, pColor, false);
			ctx.disableScissor();
			...
			ctx.text(font, "select or add an entry", editX, listY + 30, Theme.TEXT_DIM, false);
	...
	private void drawFontDropdown(GuiGraphicsExtractor ctx, int mx, int my) {
		...
		for (int i = 0; i < TextHud.FONT_NAMES.length; i++) {
			...
			ctx.text(font, TextHud.FONT_NAMES[i], rect[0] + 8, iy + (itemH - font.lineHeight) / 2 + 1, textColor, false);
		}
		ctx.disableScissor();
	}
	...
	private void fillDot(GuiGraphicsExtractor ctx, int cx, int cy, int r, int color) {
