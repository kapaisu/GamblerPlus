...
public final class TrackerScreen extends Screen {
	...
	public void render(GuiGraphics ctx, int mx, int my, float delta) {
		...
		pose.pushPose();
		pose.translate(0f, (1f - mainE) * -32f, 0f);
		...
		if (sidePanelsFit) {
			pose.pushPose();
			pose.translate(0f, (1f - sessionsE) * -40f, 0f);
			drawSessionsPanel(ctx, lmx, lmy);
			pose.popPose();

			pose.pushPose();
			pose.translate(0f, (1f - verifyE) * -40f, 0f);
	...
	public boolean mouseClicked(double _mx0, double _my0, int _btn) {
		...
		for (int i = 0; i < payBackHits.size(); i++) {
			...
			if (mx >= h[0] && mx < h[2] && my >= h[1] && my < h[3]) {
				...
				Minecraft.getInstance().setScreen(new ChatScreen("/pay " + player + " " + amount));
				return true;
			}
		}
		...
		for (int i = 0; i < pastSessionHits.size(); i++) {
			...
			if (mx >= h[0] && mx < h[2] && my >= h[1] && my < h[3]) {
				...
				Minecraft.getInstance().setScreen(new SessionDetailScreen(() -> pick, false));
				return true;
			}
		}
