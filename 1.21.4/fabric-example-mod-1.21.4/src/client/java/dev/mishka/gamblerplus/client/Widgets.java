...
public final class Widgets {
	...
	public static boolean button(GuiGraphics ctx, Font font, String label,
		...
		ctx.drawString(font, label, x + (w - lw) / 2, y + (h - font.lineHeight) / 2, textColor, false);
		return hover;
	}

	public static boolean flatButton(GuiGraphics ctx, Font font, String label,
	...
	public static void panel(GuiGraphics ctx, int x1, int y1, int x2, int y2) {
		Theme.roundPanel(ctx, x1, y1, x2, y2, 6, Theme.BG, Theme.PANEL_LINE);
	}

	public static void header(GuiGraphics ctx, Font font, int x, int y, int w, String title) {
		Theme.roundRect(ctx, x, y, x + w, y + 24, 6, Theme.SURFACE);
		ctx.fill(x + 1, y + 23, x + w - 1, y + 24, Theme.PANEL_LINE);
		ctx.drawString(font, title, x + 12, y + 9, Theme.TEXT, false);
	}

	public static void textField(GuiGraphics ctx, Font font, int x, int y, int w, int h,
		...
		ctx.drawString(font, shown, x + 6, textY, color, false);
