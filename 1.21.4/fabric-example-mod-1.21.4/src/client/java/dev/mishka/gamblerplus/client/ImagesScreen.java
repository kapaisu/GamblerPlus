...
public final class ImagesScreen extends Screen {
	...
	public ImagesScreen() {
		super(Component.literal("Images"));
	}
	...
	public void render(GuiGraphics ctx, int mxRaw, int myRaw, float delta) {
		...
		ctx.drawString(font, "drop PNG/JPG files in the folder, then refresh",
		...
		ctx.drawString(font, "IN FOLDER", leftX + 6, leftY + 3, Theme.TEXT_DIM, false);
		...
		int rowY = leftY + 14;
		if (files.isEmpty()) {
			ctx.drawString(font, "no images found", leftX + 8, leftY + 22, Theme.TEXT_DIM, false);
		} else {
			...
			for (String f : files) {
				...
				ctx.drawString(font, f, leftX + 8, rowY + (ROW_H - font.lineHeight) / 2 + 1, Theme.TEXT, false);
				ctx.drawString(font, "+ add", leftX + leftW - 32, rowY + (ROW_H - font.lineHeight) / 2 + 1, hover ? Theme.GAIN : Theme.TEXT_DIM, false);
		...
		ctx.drawString(font, "ON YOUR HUD", rightX + 6, leftY + 3, Theme.TEXT_DIM, false);

		entryRects.clear();
		int erY = leftY + 14;
		if (layout.imageEntries.isEmpty()) {
			ctx.drawString(font, "no images added yet", rightX + 8, leftY + 22, Theme.TEXT_DIM, false);
		} else {
			...
			for (int i = 0; i < layout.imageEntries.size(); i++) {
				...
				ctx.drawString(font, (i + 1) + ". " + en.file, rightX + 8, erY + (ROW_H - font.lineHeight) / 2 + 1, isSel ? Theme.TEXT : Theme.TEXT_MUTED, false);
