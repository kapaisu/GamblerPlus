...
public final class TextHud {
	...
	public static void drawAll(GuiGraphics ctx, Font font, HudLayout layout, boolean editOutline) {
		...
		if (editOutline && layout.textEntries.isEmpty()) {
			...
			ctx.drawString(font, "no text entries", x + 4, y + 5, Theme.TEXT_DIM, false);
		}
	}

	public static void drawEntry(GuiGraphics ctx, Font font, HudLayout.TextEntry entry, boolean editOutline) {
		...
		var pose = ctx.pose();
		pose.pushPose();
		pose.translate(x + 4f, y + 3f, 0f);
		pose.scale(entry.scale, entry.scale, 1f);
		...
		ctx.drawString(font, c, 0, 0, color, false);
