...
public final class RakebackScreen extends Screen {
	...
	public RakebackScreen(Config config, SessionManager sessions) {
		super(Component.literal("Rakeback"));
		this.config = config;
	...
	public void render(GuiGraphics ctx, int mxRaw, int myRaw, float delta) {
		...
		var pose = ctx.pose();
		pose.pushPose();
		pose.translate(0f, (1f - eased) * -14f, 0f);
		...
		ctx.drawString(font, "rakeback owed", px + 14, py + 9, Theme.TEXT, false);

		String rateStr = "rate " + formatPct(config.rakebackPct());
		int rw = font.width(rateStr);
		ctx.drawString(font, rateStr, px + PANEL_W - rw - 14, py + 9, Theme.BRAND, false);
		...
		ctx.drawString(font, "PLAYER",   col1, headerY, Theme.TEXT_DIM, false);
		ctx.drawString(font, "RECEIVED", col2, headerY, Theme.TEXT_DIM, false);
		ctx.drawString(font, "OWED",     col3, headerY, Theme.TEXT_DIM, false);
		...
		if (entries.isEmpty()) {
			...
			for (int i = 0; i < max; i++) {
				...
				ctx.drawString(font, e.getKey(),                 col1, rowY, Theme.TEXT, false);
				ctx.drawString(font, AmountFormat.pretty(received), col2, rowY, Theme.GAIN, false);
				ctx.drawString(font, AmountFormat.pretty(owed),     col3, rowY, Theme.LOSS, false);
				...
				ctx.drawString(font, "paid", btnX + (btnW - lw) / 2, btnY + (btnH - font.lineHeight) / 2 + 1,
		...
		ctx.drawString(font, totalStr, px + 14, py + PANEL_H - 22, entries.isEmpty() ? Theme.TEXT_DIM : Theme.LOSS, false);
	...
	@Override
	public void onClose() {
		Minecraft.getInstance().setScreen(new TrackerScreen(config, GamblerPlusClient.STATS));
	}
