...
public final class TimerHud {
	...
	public static void draw(GuiGraphics ctx, Font font, HudLayout layout, boolean editOutline) {
		...
		var pose = ctx.pose();
		pose.pushPose();
		pose.translate(x + 6f, y + 4f, 0f);
		pose.scale(layout.timerScale, layout.timerScale, 1f);
		ctx.drawString(font, "TIMER", 0, 0, Theme.BRAND, false);
		String time = timer.active()
				? TimeParse.prettyDuration(timer.remainingMs())
				: "0:00";
		int color = timer.active() ? Theme.TEXT : Theme.TEXT_DIM;
		ctx.drawString(font, time, 0, 12, color, false);
		pose.popPose();
	}
}
