...
public final class NetGraph {
	...
	public static void draw(GuiGraphics ctx, Font font,
		...
		ctx.drawString(font, AmountFormat.signed(maxNet), x + 4, chartY - 1, Theme.TEXT_DIM, false);
		ctx.drawString(font, AmountFormat.signed(minNet), x + 4, chartY + chartH - font.lineHeight, Theme.TEXT_DIM, false);
		...
		if (hovered >= 1) {
			...
			ctx.drawString(font, time, tx + 5, ty + 4, Theme.TEXT_DIM, false);
			ctx.drawString(font, netStr, tx + 5, ty + 4 + font.lineHeight, Theme.color(cum[hovered]), false);
			ctx.drawString(font, delta, tx + 5, ty + 4 + font.lineHeight * 2, e.incoming() ? Theme.GAIN : Theme.LOSS, false);
		}
	}

	private static void drawThickLine(GuiGraphics ctx, int x0, int y0, int x1, int y1, int color) {
