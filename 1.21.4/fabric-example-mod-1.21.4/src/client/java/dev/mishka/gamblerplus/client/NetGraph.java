...
public final class NetGraph {
	...
	public static void draw(GuiGraphics ctx, Font font,
		...
		if (log.isEmpty()) {
			...
			int sw = font.width(s);
			ctx.drawString(font, s, x + (w - sw) / 2, y + (h - font.lineHeight) / 2, Theme.TEXT_DIM, false);
			return;
		}
		...
		ctx.drawString(font, AmountFormat.signed(maxV), x + 4, chartY - 1, Theme.TEXT_DIM, false);
		ctx.drawString(font, AmountFormat.signed(minV), x + 4, chartY + chartH - font.lineHeight, Theme.TEXT_DIM, false);
		if (zeroY - chartY > font.lineHeight + 2 && chartBottom - zeroY > font.lineHeight + 2) {
			ctx.drawString(font, "0", x + 4, zeroY - font.lineHeight / 2, Theme.TEXT_DIM, false);
		}
		...
		if (hoverBucket >= 0) {
			...
			ctx.drawString(font, timeStr,  tx + 5, ty + 4, Theme.TEXT_DIM, false);
			ctx.drawString(font, openStr,  tx + 5, ty + 4 + font.lineHeight,     Theme.TEXT, false);
			ctx.drawString(font, hiStr,    tx + 5, ty + 4 + font.lineHeight * 2, Theme.GAIN, false);
			ctx.drawString(font, loStr,    tx + 5, ty + 4 + font.lineHeight * 3, Theme.LOSS, false);
			ctx.drawString(font, closeStr, tx + 5, ty + 4 + font.lineHeight * 4, trend, false);
		}
	}
}
