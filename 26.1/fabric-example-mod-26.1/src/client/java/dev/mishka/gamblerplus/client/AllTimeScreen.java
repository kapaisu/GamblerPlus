...
public final class AllTimeScreen extends Screen {
	...
	public void extractRenderState(GuiGraphicsExtractor ctx, int mxRaw, int myRaw, float delta) {
		...
		ctx.text(font, countLabel, countX, barY + (searchH - font.lineHeight) / 2 + 1, Theme.TEXT_DIM, false);
		...
		if (filtered.isEmpty()) {
			...
			int sw = font.width(s);
			ctx.text(font, s, listX + (listW - sw) / 2, listTop + (listBot - listTop) / 2 - 4, Theme.TEXT_DIM, false);
			pose.popMatrix();
			return;
		}
		...
		for (int i = scroll; i < filtered.size(); i++) {
			...
			ctx.text(font, time, listX + 8, drawY, Theme.TEXT_DIM, false);
			ctx.text(font, dir, listX + 76, drawY, dirColor, false);
			ctx.text(font, p.player(), listX + 100, drawY, Theme.TEXT, false);
			int xBtnX = listX + listW - 16;
			int amtX = xBtnX - 8 - aw;
			ctx.text(font, amt, amtX, drawY, dirColor, false);
			String mult = multLabels.get(p);
			if (mult != null) {
				int mw = font.width(mult);
				ctx.text(font, mult, amtX - mw - 4, drawY, Theme.WARN, false);
			}
			boolean xHover = mx >= xBtnX - 2 && mx < xBtnX + 10 && my >= drawY - 1 && my < drawY + ROW_H - 2;
			ctx.text(font, "x", xBtnX, drawY, xHover ? Theme.LOSS : Theme.TEXT_DIM, false);
