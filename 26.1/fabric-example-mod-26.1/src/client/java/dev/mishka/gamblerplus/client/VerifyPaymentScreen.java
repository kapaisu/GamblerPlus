...
public final class VerifyPaymentScreen extends Screen {
	...
	public VerifyPaymentScreen(String recipient, long amount, String rawCommand) {
		super(Minecraft.getInstance(), Minecraft.getInstance().font, Component.literal("Confirm payment"));
	...
	public void extractRenderState(GuiGraphicsExtractor ctx, int mxRaw, int myRaw, float delta) {
		...
		var pose = ctx.pose();
		pose.pushMatrix();
		pose.translate(0f, (1f - eased) * -18f);
		...
		ctx.text(font, title, px + (PANEL_W - tw) / 2, py + 14, Theme.TEXT, false);
		...
		ctx.text(font, warn, px + (PANEL_W - ww) / 2, py + PANEL_H - 60, Theme.TEXT_MUTED, false);
	...
	private void labelValue(GuiGraphicsExtractor ctx, int x, int y, String label, String value, int color) {
		ctx.text(font, label, x, y, Theme.TEXT_DIM, false);
		ctx.text(font, value, x + 42, y, color, false);
	}

	private void drawButton(GuiGraphicsExtractor ctx, String label, int x, int y, int w, int h, int mx, int my, int accent) {
