...
public final class AuctionScreen extends Screen {
	...
	public void extractRenderState(GuiGraphicsExtractor ctx, int mxRaw, int myRaw, float delta) {
		...
		ctx.text(font, heldLabel, contentX + 10, itemBoxY + (itemBoxH - font.lineHeight) / 2, Theme.TEXT, false);
		int snapW = 84;
		...
		ctx.text(font, "time (e.g. 30s, 2m, 1m30s)", contentX, timeLabelY, Theme.TEXT_DIM, false);
		...
		if (live) {
			...
			int bidsY = stateY + 14;
			ctx.text(font, "TOP BIDS", contentX, bidsY, Theme.TEXT_DIM, false);
			List<Auction.Bid> bids = auction.topBids(5);
			int rowY = bidsY + 12;
			if (bids.isEmpty()) {
				ctx.text(font, "no bids yet", contentX, rowY, Theme.TEXT_MUTED, false);
			} else {
				for (Auction.Bid b : bids) {
					int accent = b == bids.get(0) ? Theme.GAIN : Theme.TEXT;
					ctx.text(font, b.player(), contentX, rowY, accent, false);
					String amt = AmountFormat.pretty(b.amount());
					int aw = font.width(amt);
					ctx.text(font, amt, contentX + fieldW - aw, rowY, accent, false);
					rowY += 12;
				}
			}
		} else {
			...
			if (w != null) {
				ctx.text(font, "winner", contentX, stateY, Theme.TEXT_DIM, false);
				ctx.text(font, w.player() + "  " + AmountFormat.pretty(w.amount()),
						contentX, stateY + 12, Theme.GAIN, false);
				ctx.text(font, "for " + auction.item() + (auction.itemCount() > 1 ? " x" + auction.itemCount() : ""),
