...
public final class AuctionHud {
	...
	public static void draw(GuiGraphicsExtractor ctx, Font font, HudLayout layout, boolean editOutline) {
		...
		ctx.text(font, "Gambler", 0, 0, Theme.TEXT, false);
		int brandW = font.width("Gambler ");
		ctx.text(font, "Plus", brandW, 0, Theme.BRAND, false);
		int psuffix = font.width("Gambler Plus ");
		ctx.text(font, "auction", psuffix, 0, Theme.TEXT_DIM, false);
		...
		if (live) {
			...
			ctx.text(font, trim(font, item, BASE_W - 20), 0, y0, Theme.TEXT, false);
			...
			if (bids.isEmpty()) {
				...
				for (int i = 0; i < bids.size(); i++) {
					...
					ctx.text(font, (i + 1) + "." + b.player(), 0, rowY, color, false);
					String amt = AmountFormat.pretty(b.amount());
					int aw = font.width(amt);
					ctx.text(font, amt, BASE_W - 18 - aw, rowY, color, false);
					rowY += LINE_H;
				}
			}
		} else if (linger) {
			...
			ctx.text(font, secs, BASE_W - 18 - sw, y0, Theme.TEXT_DIM, false);
			if (w != null) {
				ctx.text(font, w.player(), 0, y0 + LINE_H + 2, Theme.TEXT, false);
				String item = auction.item();
				if (auction.itemCount() > 1) item = item + " x" + auction.itemCount();
				ctx.text(font, "won " + trim(font, item, BASE_W - 34), 0, y0 + LINE_H * 2 + 2, Theme.TEXT_MUTED, false);
				String amt = "for " + AmountFormat.pretty(w.amount());
				ctx.text(font, amt, 0, y0 + LINE_H * 3 + 4, Theme.GAIN, false);
			} else {
